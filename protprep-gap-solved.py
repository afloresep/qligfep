...
class Run(object):
    ...
    def write_tmpPDB(self):
        ...
        # Open and read tmp
        with open(self.prot[:-4] + '_tmp.pdb', 'r') as tmp_file:
            tmp_lines = tmp_file.readlines()

        # Read TER/GAP lines from self.prot
        with open(self.prot, 'r') as prot_file:
            for line in prot_file:
                if 'TER' in line or 'GAP' in line:
                    # Append lines to tmp
                    tmp_lines.append(line)

        # Sort lines by atom number
            tmp_lines.sort(key=lambda x: str(x[21:26]))

        # Escribir las líneas ordenadas de vuelta al archivo tmp.pdb
        with open(self.prot[:-4] + '_tmp.pdb', 'w') as tmp_file:
